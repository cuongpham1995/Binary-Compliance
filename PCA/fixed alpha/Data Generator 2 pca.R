...
fIV = function(y,comb, alpha0, alpha1, mu, sigma, gamma){
  w(comb, alpha0, alpha1)*dnorm(y,mu,sigma)/gamma
}
...
gen.compl.data = function(seed.l, seed.y, sample.size = 500, alpha_n1 = 0, alpha_p1 = 0){
  ...
  pca.n = prcomp(cbind(y_n1, L1_n1, L2_n1), retx = T, center = T, scale. = T)
  
  
  for(i in 1:n_n1){
   
    temp_y_n1<-rnorm(5000, 1+2*(L1_n1[i] + L2_n1[i] ), 0.5)
    pred.input = matrix(c( temp_y_n1, rep(L1_n1[i], 5000), rep(L2_n1[i], 5000) ), ncol = 3)
    colnames(pred.input) = c("y_n1", "L1_n1", "L2_n1")
    pred.res = predict(pca.n, newdata = pred.input)[,1]
    gamma_n1[i]<-mean(w(temp_y_n1,alpha0_n1,alpha_n1))
  }
  
  pca.p = prcomp(cbind(y_p1, L1_p1, L2_p1), retx = T, center = T, scale. = T)
  
  for(i in 1:n_p1){
    temp_y_p1<-rnorm(5000, 1-0*(L1_p1[i] + L2_p1[i] ),0.5)
    pred.input = matrix(c(temp_y_p1, rep(L1_p1[i],5000), rep(L2_n1[i],5000)), ncol = 3)
    colnames(pred.input) = c("y_p1", "L1_p1", "L2_p1")
    pred.res = predict(pca.p, newdata = pred.input)
  ...
  pca.n1 = predict(pca.n)[,1]
  pca.p1 = predict(pca.p)[,1]
  ...
  for(i in 1:length(mu_n1)){
    ...
    x = rnorm(8000,1.5,2)
    #process of sampling
    criteria = fIV(y = x, comb  = pca.n1[i], alpha0_n1, alpha_n1, mu_n1[i], 0.5, temp.gamma_n1[i])/(M*dnorm(x,1.5,2))
  ...
  for (i in 1:length(mu_p1)) {
    ...
    x = rnorm(8000,1.5,2)
    #process of sampling
    criteria = fIV(y = x, comb = pca.p1[i], alpha0_p1, alpha_p1, mu_p1[i],0.5, temp.gamma_p1[i])/(M*dnorm(x,1.5,2))
    chosen_sam = x[u < criteria]
