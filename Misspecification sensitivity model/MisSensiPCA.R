...
#All nuisance parameters are correctly specified
#########################################################################
...
w2<-function(y, x1, x2, alpha0, alpha1, alpha2, alpha3){
  expit(alpha0 +alpha1*y + alpha2*x1 + alpha3*x2)
}
...
fIV = function(y, x1, x2, alpha0, alpha1, alpha2, alpha3, mu, sigma, gamma){
  w2(y=y, x1=x1,x2=x2, alpha0=alpha0,alpha1=alpha1, alpha2=alpha2, alpha3=alpha3)*dnorm(y,mu,sigma)/gamma
}
...
gen.compl.data = function(seed.l, seed.y, sample.size = 500, alpha_n1 = 0, alpha_p1 = 0){
  ...
  for(i in 1:n_n1){
    ...
    gamma_n1[i]<-mean(w2(y = temp_y_n1, x1 = L1_n1[i], x2 = L2_n1[i], alpha0_n1,alpha_n1, alpha2 = alpha2_n1, alpha3 = alpha3_n1))
  }
  
  for(i in 1:n_p1){
    temp_y_p1<-rnorm(5000, 1-0*(L1_p1[i] + L2_p1[i] ),0.5)
    gamma_p1[i]<-mean(w2( y = temp_y_p1, x1 = L1_p1[i], x2 = L2_p1[i], alpha0_p1,alpha_p1, alpha2 = alpha2_p1, alpha3 = alpha3_p1))
  }
  ...
  for(i in 1:length(mu_n1)){
    ...
    x = rnorm(8000,1.5,2)
    #process of sampling
    criteria = fIV(x, x1 = L4_n1[i,1], x2 = L4_n1[i,2], alpha0_n1, alpha_n1, alpha2 =alpha2_n1, alpha3 = alpha3_n1, mu_n1[i], 0.5, temp.gamma_n1[i])/(M*dnorm(x,1.5,2))
  ...
  for (i in 1:length(mu_p1)) {
    ...
    x = rnorm(8000,1.5,2)
    #process of sampling
    criteria = fIV(x, x1 = L4_p1[i,1], x2 = L4_p1[i,2], alpha0_p1, alpha_p1, alpha2 = alpha2_p1, alpha3 = alpha3_p1, mu_p1[i],0.5, temp.gamma_p1[i])/(M*dnorm(x,1.5,2))
    chosen_sam = x[u < criteria]
...
bi.sim.correct = function(seed.l, seed.y,true.al_n1, true.al_p1, alpha_n1 = 0, alpha_p1 = 0,alpha0_n1 = 0, alpha0_p1 = 0, size){
  tryCatch({
    require(locClass)
    require(dplyr)
    #require(DynTxRegime)
    ...
    pca.n = prcomp( ~ outcome + L1 + L2, data = dat[dat$Z == -1 & dat$A == - 1,] , retx = T, center = T, scale. = T)
    pca.n.pred = predict(pca.n, newdata = dat[,c("outcome", "L1", "L2")])[,1]
    
    pca.p = prcomp( ~ outcome + L1 + L2, data = dat[dat$Z == 1 & dat$A ==  1,] , retx = T, center = T, scale. = T)
    pca.p.pred = predict(pca.p, newdata = dat[,c("outcome", "L1", "L2")])[,1]
    ...
    dat$wd = ifelse(dat$Z == -1, w(pca.n.pred, alpha0_n1, alpha_n1), w(pca.p.pred , alpha0_p1, alpha_p1))
    ...
    for(i in 1:nrow(dat.PI)){
      ...
      pca.p.input = matrix(c(temp.yp, rep(dat.PI$L1[i], 5000), rep(dat.PI$L2[i],5000)), ncol = 3)
      colnames(pca.p.input) = c("outcome", "L1", "L2")
      pca.p.pred2 = predict(pca.p, newdata = pca.p.input)
      Ey.wp[i] = mean(temp.yp*w(pca.p.pred2, alpha0 = alpha0_p1, alpha1 = alpha_p1)) #calculate E[Yw+| A = Z = 1, L]
      p.gammap[i] = mean(w(pca.p.pred2, alpha0 = alpha0_p1, alpha1 = alpha_p1)) #calculate gamma+ (E[w+| A = Z = 1, L])
    }
    ...
    for(i in 1:nrow(dat.PI)){
      ...
      pca.n.input = matrix(c(temp.yn, rep(dat.PI$L1[i], 5000), rep(dat.PI$L2[i],5000)), ncol = 3)
      colnames(pca.n.input) = c("outcome", "L1", "L2")
      pca.n.pred2 = predict(pca.n, newdata = pca.n.input)
      Ey.wn[i] = mean(temp.yn*w(pca.n.pred2 ,alpha0 = alpha0_n1, alpha1 = alpha_n1)) #calculate E[Yw-|A = Z = -1, L]
      p.gamman[i] = mean(w(pca.n.pred2, alpha0 = alpha0_n1, alpha1 = alpha_n1)) #calculate gamma- (E[w-|A= Z = -1, L]) 
    }
    ...
    dat1.test = gen.compl.data(seed.l = seed.l*2, seed.y = seed.y, alpha_n1 = true.al_n1, alpha_p1 = true.al_p1, sample.size = size)
    ...
    #Part II: Predicting on the test data set.
    ...
    mod.multi2 = multinom(A ~ L1 + L2 + Z, data = test.df)
    faz2 = predict(mod.multi2, type = "prob")
    test.df$faz = ifelse(test.df$Z == 1,faz2[,3],faz2[,1])
    
    pca.n = prcomp( ~ outcome + L1 + L2, data = test.df[test.df$Z == -1 & test.df$A == - 1,] , retx = T, center = T, scale. = T)
    pca.n.pred = predict(pca.n, newdata = test.df[,c("outcome", "L1", "L2")])[,1]
    
    pca.p = prcomp( ~ outcome + L1 + L2, data = test.df[test.df$Z == 1 & test.df$A ==  1,] , retx = T, center = T, scale. = T)
    pca.p.pred = predict(pca.p, newdata = test.df[,c("outcome", "L1", "L2")])[,1]
    
    
    #calcuate the weight w+(Y) and w-(Y)
    test.df$wd = ifelse(test.df$Z == -1, w(pca.n.pred, alpha0_n1, alpha_n1), w(pca.p.pred, alpha0_p1, alpha_p1))
    ...
    for(i in 1:nrow(test.df)){
      temp.yp = rnorm(5000, coef.p[1] , 0.5) #the true model for yp (y positive) has only intercept
      pca.p.input = matrix(c(temp.yp, rep(test.df$L1[i], 5000), rep(test.df$L2[i],5000)), ncol = 3)
      colnames(pca.p.input) = c("outcome", "L1", "L2")
      pca.p.pred2 = predict(pca.p, newdata = pca.p.input)
      Ey.wp[i] = mean(temp.yp*w(pca.p.pred2,alpha0 = alpha0_p1, alpha1 = alpha_p1)) #calculate E[Yw+| A = Z = 1, L]
      p.gammap[i] = mean(w(pca.p.pred2, alpha0 = alpha0_p1, alpha1 = alpha_p1)) #calculate gamma+ (E[w+| A = Z = 1, L])
    ...
    for(i in 1:nrow(test.df)){
      ...
      pca.n.input = matrix(c(temp.yn, rep(test.df$L1[i], 5000), rep(test.df$L2[i],5000)), ncol = 3)
      colnames(pca.n.input) = c("outcome", "L1", "L2")
      pca.n.pred2 = predict(pca.n, newdata = pca.n.input)
      Ey.wn[i] = mean(temp.yn*w(pca.n.pred2 ,alpha0 = alpha0_n1, alpha1 = alpha_n1)) #calculate E[Yw-|A = Z = -1, L]
      p.gamman[i] = mean(w(pca.n.pred2, alpha0 = alpha0_n1, alpha1 = alpha_n1)) #calculate gamma- (E[w-|A= Z = -1, L]) 
...
#bi.sim.correct(seed.l = 2323, seed.y = 3432, true.al_n1 = 0.5, true.al_p1 = -0.5, alpha_n1 = 0.5, alpha_p1 = -0.5, size = 500)
###################################################################
############# Simulation ##########################################
###################################################################
...
temp.df = mapply(bi.sim.correct, seed.l = seed.l, seed.y = seed.y, 
...
correct <- paste(true.al_p1, true.al_n1, size, "sensitivity_mod_misPCA" ,sep = " ")
