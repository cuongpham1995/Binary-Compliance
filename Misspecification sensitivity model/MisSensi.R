...
bi.sim.correct = function(seed.l, seed.y, true.al_n1, true.al_p1, alpha_n1 = 0, alpha_p1 = 0,alpha0_n1 = 0.5, alpha0_p1 = 0.5, size){
  tryCatch({
    require(locClass)
    require(dplyr)
    require(DynTxRegime)
    ...
    # pd1<-mean(dat$A[dat$Z == 1] == 1)
    # pd2<-mean(dat$A[dat$Z == -1] == -1)
    ...
    hal.mod1 = fit_hal(X = matrix(c(rep(1, length(test.df$L1)),test.df$L1, test.df$L2, test.df$Z, test.df$PI), ncol = 5),
                       Y = I(test.df$outcome*test.df$wd2/(test.df$faz*test.df$gamma + 2*(test.df$gamma == 0)) ), family = "gaussian", yolo = F)
    
    test.df$EAYw = predict(hal.mod1, new_data = matrix(c(rep(1, length(test.df$L1)),test.df$L1, test.df$L2, test.df$Z, test.df$PI), ncol = 5)) 
    
    test.df$EAYwn = predict(hal.mod1, new_data = matrix(c(rep(1, length(test.df$L1)),test.df$L1, test.df$L2, rep(-1, length(test.df$L1)), test.df$PI), ncol = 5))
    
    test.df$EAYwp = predict(hal.mod1, new_data = matrix(c(rep(1, length(test.df$L1)),test.df$L1, test.df$L2, rep(1, length(test.df$L1)), test.df$PI), ncol = 5))
...
correct <- paste(true.al_p1, true.al_n1, size, "sensitivity_mod_mis" ,sep = " ")
