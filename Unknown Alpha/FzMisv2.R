#In this scenario, we misspecify the nuisance parameter f(Z|X)
#In this one, we incorrectly specify the alpha_0
#########################################################################
...
gen.compl.data = function(seed.l, seed.y, sample.size = 500, alpha_n1 = 0, alpha_p1 = 0){
  require(dplyr)
  #generate the two covariates from uniform distribution [-1,1] with 500 samples
  n = sample.size
  
  L = cov.gen(n = n, seed = seed.l)[,1:2] 
  U = cov.gen(n = n, seed = seed.l)[,3]
  #generate confounder U from a bridge distribution with the scale parameter 0.5. 
  
  #Randomly assigned the treatement Z. Z = -1 denotes taking treatment negative, Z = 1 denotes taking treatment positive 
  #Z is generated using a bernoulli distribution with p = 0.5
  #Z = rbinom(n,1,0.5)
  Z = rbinom(n,1,expit(0.5*L[,1]+0.5*L[,2]))
  Z = ifelse(Z == 0,1,-1)
...
###############################################################################################
###################### Function for fixed alpha ###############################################
###############################################################################################
#true.al_n1 = 0.5
#true.al_p1 = -0.5

bi.sim.fz.mis = function(seed.l, seed.y, alpha_n1 = 0, true.al_n1, true.al_p1, alpha_p1 = 0, alpha0_n1 = 0.5, alpha0_p1 = 0.5, size){
  tryCatch({
    require(locClass)
    require(dplyr)
    require(DynTxRegime)
...
temp.df = mapply(bi.sim.fz.mis, seed.l = seed.l, seed.y = seed.y, 
...
correct <- paste(true.al_p1, true.al_n1, size, "fz mis" ,sep = " ")
