...
complier.cov = function(seed, alpha_n1, alpha_p1,ps4 = 0.3, col, kernel.type = "linear"){
 set.seed(seed)
    require(locClass)
    require(dplyr)
    require(matrixStats)
  ...
  #  dat$f.a.z.x = ifelse(dat[,col] == 1, p.1, p.0)
    mu.S4 = mean(dat$gamma[dat$gamma != 0])    
 ...
 # return(list(population =mean(dat[,col]),
  #            complier = mean(dat[dat$gamma != 0,col]*(dat$gamma[dat$gamma != 0]/mu.S4)) ) )
     ...
     return(list(comp.mean = mean(dat[dat$gamma != 0,col]*(dat$gamma[dat$gamma != 0]/mu.S4)),
                 comp.sd = sd(dat[dat$gamma != 0,col]*(dat$gamma[dat$gamma != 0]/mu.S4)) ))   
}
...
alpha.grid = expand.grid(alpha_n = c(0.0000, 0.5, 1), alpha_p = c(0.0000, 0.5, 1))
alpha.grid = alpha.grid[c(1,9),]

#boxplot(dat.coc.bi$emotional_baseline[dat.coc.bi$DaysCoc.bi == -1])
n.sim = 100

covarites = c("L1", "L2", "L3", "L4", "L5", "L6", "L7")
tab = matrix(nrow = length(covarites), ncol = nrow(alpha.grid))
...
for(i in 1:n.sim){
  #print(i)
  temp = mapply(seed = i , complier.cov, alpha_n1 = alpha.grid$alpha_n, alpha_p1 = alpha.grid$alpha_p, col = covarites[j])
  
  L1.mean[i,] = unlist(temp[1,])
  L1.sd[i,] = unlist(temp[2,])
}

tab[j,] = paste0( round(apply(L1.mean,2,mean),2) , "(", round(apply(L1.sd,2,mean),2),")", sep = "")
}
...
population = dat.coc.bi %>% mutate(complier = I(trt == compl)) %>% summarise(Race = mean.sd(aa), Sex = mean.sd(male), higheduc = mean.sd(higheduc),  smoke_baseline = mean.sd(smoke_baseline), Txready = mean.sd(TxReadiness_baseline), Health = mean.sd(GH_baseline), Emo = mean.sd(emotional_baseline))
