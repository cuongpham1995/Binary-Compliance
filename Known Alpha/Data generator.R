#try this
#########################################################################
#In this file, we generate the data set that we use in our simulation
#########################################################################


#calculate expit function
expit<-function(x){
  exp(x)/(1+exp(x))
}

#generate the w(alpha1, alpha0, y)
...
#We denote L be the set of covariates and U be the unmeasured confounder; A be the compliance level; Z be the treatment (IV), and Y be the outcome
#There will be two treatments: negative and positive
#There are three compliance level: negative (-1), zero (0), positive (1)

#cov.gen is used to generate the covariates and the unmeasured confounders
#it returns a data frame that contains 2 covariates and one unmeasured confounders
...
#gen.comp.data generate the whole data set. It uses the function cov.gen above to generate the covariate and the unmeasured confounders.
#It also creates the treatment. Based on the covariates, unmeasured confounders, and the treatmet assigned, we create the compliance levels.
#Using covraites, unmeasured confounders, treatment assigned and compliance level, we create the outcome
