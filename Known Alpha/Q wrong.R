#####################################################################
## In this scenario we consider the case that we misspecify Q^z(X) ##
#####################################################################

bi.sim.Q.mis = function(seed.l, seed.y, alpha_n1 = 0, alpha_p1 = 0,alpha0_n1 = 0, alpha0_p1 = 0, size){
  tryCatch({
    require(locClass)
    require(dplyr)
    require(DynTxRegime)
    ...
    #propensity score f(Z|X) is correctly specified
    ...
    #multiple robust estimator for E[Y+|PI = S4, L]
    ...
    #### Obtain the predicted treatment assignment ####################
    
    #for the proposed method (IPW)
    ...
    #for the method proposed by Tchetgen and Cui
    ...
    #for the method proposed by the multiply robust estimator
    ...
    #for the multiply robust method
    ...
    #correct classification for the IPW method
    ...
    #correct classification for the OWL method
    ...
    #correct classificaiton for the method by Cui and Tchetgen
    ...
    #correct classification for the multiply robust estimator
    ...
    ########################### Calculate value function using the multiply robust estimator ####################
    #############################################################################################################
    
    test.df = dat.test
    
    #correctly specify f(Z|X)
    ...
    ################ calculate the value functions using the non-robust formula ################
...
bi.sim.Q.mis(342,585,alpha_n1 = 0.5, alpha_p1 = 0.5, size = 2000)
